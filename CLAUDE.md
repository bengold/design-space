...
- **Claude Code plugin** — `.claude-plugin/plugin.json` registers the MCP server (via `${CLAUDE_PLUGIN_ROOT}/mcp-server/index.mjs`, with `DESIGN_SPACE_ROOT=${CLAUDE_PROJECT_DIR}`) and the skill at `skills/design-space/SKILL.md`. Install from the repo with `claude plugin add ./.claude-plugin`.
- **Cursor / Claude Desktop** — `.cursor/mcp.json` is gitignored; generate it with `npx design-space mcp install <cursor|cursor-global|claude-desktop>`. Example shape at `.cursor/mcp.example.json`.
