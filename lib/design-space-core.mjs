...
// Resolves on either "answered" (user submitted) or "dismissed" (user
// canceled / closed / idled out). Callers inspect q.status to branch.
...
// Block until events.jsonl picks up new entries past `since`, then return the
// batch. Mirrors Agentation's `watch_annotations` — agents loop on this to
// get a push-style cadence over a file-based event log. Polls at 1Hz (cheap
// — we just read a small jsonl file each tick).
export async function waitForEvents(name, { since = null, timeoutSec = 600 } = {}) {
...
export function readDomSnapshot(name) {
